...
export default async (req: Request, res: Response, next: NextFunction) => {
  if (!req.body.refresh) {
    const {statusCode, body} = formatResponse(httpStatus.BAD_REQUEST,  errors.refreshTokenNotPresent, false )
  ...
  try {
    ...
    if (
      ...
      const {statusCode, body} = formatResponse(httpStatus.UNAUTHORIZED,  errors.invalidTokenType, false )
      return res.status(statusCode).json(body);
    }
    ...
    if (value) {
      ...
      const {statusCode, body} =  formatResponse(httpStatus.UNAUTHORIZED, errors.refreshTokenUsed, false);
      return res.status(statusCode).json(body);
    }
    ...
    console.error(error)
    const {statusCode, body} =  formatResponse(httpStatus.UNAUTHORIZED, errors.invalidJwtToken, false);
    return res.status(statusCode).json(body);
  }
}
